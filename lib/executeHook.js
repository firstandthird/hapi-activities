...
module.exports = function(server, settings, collection, hook) {
  const launchHook = (hookLaunchedCallback) => {
    async.autoInject({
      ...
      completeHook(performActions, done) {
        ...
        async.autoInject({
          ...
          repeating(next) {
            ...
            const runAfter = later.schedule(schedule).next(1);

            server.methods.hook(hook.hookName, hook.hookData, {
              runAfter: runAfter[0],
              runEvery: hook.runEvery,
              recurringId: hook.recurringId
            });
    ...
    // don't block while the hook lifecycle is processed:
    hookLaunchedCallback();
  };
  return { launchHook };
};
