...
module.exports = (server, settings, collection, allDone) => {
  ...
  async.autoInject({
    ...
    retryHanged(logHooks, outstandingHooks, done) {
      async.each(outstandingHooks.hanged, (hangedHook, eachDone) => {
        retry(server, settings, collection, hangedHook._id, false, eachDone);
      }, done);
    },
    hooks(retryHanged, logHooks, done) {
